...
A common use-case is to have a bootstrapping application that handles user login and launches your apps. The launcher can be any application and in the general case it isn't Glue42 enabled, but the applications it launches are. This scenario can be handled both through [application configuration](#bootstrapped_apps-application_configuration) or [runtime configuration](#bootstrapped_apps-runtime_configuration) when initializing Glue42 in the respective apps.

In both cases, you must specify the name of the bootstrapping launcher app and clear the starting context passed by [**Glue42 Enterprise**](https://glue42.com/enterprise/) to the launcher. Clearing the starting context is very important, because [**Glue42 Enterprise**](https://glue42.com/enterprise/) by default will store the command line arguments with which the launcher and the launched apps were started and will try to pass them again to the launcher on restore, which in some cases may not be possible.

### Application Configuration

To specify the name of the bootstrapping launcher app in the [application configurations](../../../developers/configuration/application/index.html#application_configuration-exe) of the launched apps, use the `"launcherApp"` property of the `"details"` top-level key:

```json
// Configuration file for the launched app.
{
    "name": "my-launched-app",
    "type": "exe",
    "details": {
        "path": "%GDDIR%/PathToMyApp/",
        "launcherApp": "my-bootstrapping-launcher"
    }
}
```

*Note that the name of the launched app specified in the [application configuration](../../../developers/configuration/application/index.html#application_configuration-exe) file must be the same as the hard-coded name in the application itself.*

To clear the starting context for the bootstrapping launcher app, set the `"startingContextMode"` property of the `"details"` top-level key to `"none"`:

```json
// Configuration file for the bootstrapper.
{
    "name": "my-bootstrapping-launcher",
    "type": "exe",
    "details": {
        "path": "%GDDIR%/PathToMyLauncherApp/",
...
When [initializing](../../../getting-started/how-to/glue42-enable-your-app/net/index.html) Glue42 in your app, use the optional `InitializeOptions` object to specify configuration for your application - define the path to the bootstrapping launcher application, clear the starting context passed to the launcher, prevent the main window of a multi window app from being saved in a [Layout](../../windows/layouts/overview/index.html) or being closed when [**Glue42 Enterprise**](https://glue42.com/enterprise/) shuts down:

The following example demonstrates configuring the main window of a multi window application at runtime:

```csharp
// Options that will be used for initializing Glue42.
var initializeOptions = new InitializeOptions
    {
        AppDefinition = new AppDefinition
        {
            // Specify the full path to the bootstrapping launcher app.
            StartupFile = Path.GetFullPath("multi-window-wpf-bootstrap.cmd"),
            Title = "Bootstrapped Multi Window WPF Demo",
            // Clean or modify the arguments passed by the bootstrapper in Environment.GetCommandLineArgs().
            // By default, Glue42 Enterprise will store them and try to pass them to the bootstrapper the next time it's started.
            StartupArguments = "",
            // Prevent saving the main window in a Layout, otherwise it will be restarted on restore.
            IgnoreFromLayouts = true,
            // Prevent closing the main window when Glue42 Enterprise shuts down.
